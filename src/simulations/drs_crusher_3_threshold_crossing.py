...
stockpile_high_trigger = OperationModeTrigger(
    name="stockpile_at_or_above_high",
    check=check_stockpile_high,
    expected_next_trigger_time=_predict_time_to_high,
)
stockpile_low_trigger = OperationModeTrigger(
    name="stockpile_at_or_below_low",
    check=check_stockpile_low,
    expected_next_trigger_time=_predict_time_to_low,
)
...
def source_departure_as_rate_update(ctx: SimulationContext) -> None:
    t = ctx.engine.get_current_time()
    rate_entity = {"rate_tph": float(ctx.event.entity.get("rate_tph", 0.0))}
    ctx.engine.add_event(Event(t, ctx.component.output.component_id, "RateUpdate", rate_entity, {}))
...
def drs_crusher_simulation(visualize: bool = False) -> Engine:
    ...
    source.set_handleable_event("Departure", source_departure_as_rate_update)
